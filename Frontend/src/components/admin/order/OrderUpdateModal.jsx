...
const OrderUpdateModal = ({ isOpen, onClose, onConfirm, order }) => {
  ...
  // Reset state khi order thay đổi (để tránh lưu cache của đơn trước)
  useEffect(() => {
    setSelectedStatus(order.trangThai);
  }, [order]);
  ...
  const filteredOptions = statusOptions.filter((status) => {
    // Nếu trạng thái hiện tại là 0 (Chưa duyệt)
    if (order.trangThai === 0) {
      // Chỉ hiện: 0 (Hiện tại), 1 (Bước tiếp theo), 6 (Hủy - Nên có)
      return status.value === 0 || status.value === 1 || status.value === 6;
    }
    
    // Logic mở rộng: Chặn không cho quay lại trạng thái cũ (tùy chọn)
    // return status.value >= order.trangThai || status.value === 6;

    // Mặc định: Hiện tất cả cho các trạng thái khác
    return true; 
  });
